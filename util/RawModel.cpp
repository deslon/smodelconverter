...
void RawModel::TransformTextures(const std::vector<std::function<FbxVector2(FbxVector2)>> &transforms)
{
...
FbxVector4 RawModel::getFaceNormal(int verts[3]) const
{
    const float l0 = (vertices[verts[1]].position - vertices[verts[0]].position ).SquareLength();
    const float l1 = (vertices[verts[2]].position - vertices[verts[1]].position ).SquareLength();
    const float l2 = (vertices[verts[0]].position - vertices[verts[2]].position ).SquareLength();
    const int index = ( l0 > l1 ) ? ( l0 > l2 ? 2 : 1 ) : ( l1 > l2 ? 0 : 1 );

    const FbxVector4 e0 = vertices[verts[(index + 1) % 3]].position - vertices[verts[index]].position;
    const FbxVector4 e1 = vertices[verts[(index + 2) % 3]].position - vertices[verts[index]].position;
    if (e0.SquareLength() < FLT_MIN || e1.SquareLength() < FLT_MIN) {
        return FbxVector4(0,0,0,1);
    }
    auto result = e0.CrossProduct(e1);
    auto resultLengthSquared = result.SquareLength();
    if (resultLengthSquared < FLT_MIN) {
        return FbxVector4(0,0,0,1);
    }
    float edgeDot = std::max(-1.0f, std::min(1.0f, (float)e0.DotProduct(e1) ));
    float angle = acos(edgeDot);
    float area = resultLengthSquared / 2.0f;
    result.Normalize();
    return result * angle * area;
}

size_t RawModel::CalculateNormals(bool onlyBroken)
{
    FbxVector4 averagePos = FbxVector4(0,0,0,1);
    ...
    for (int vertIx = 0; vertIx < vertices.size(); vertIx ++) {
        ...
        if (onlyBroken && (vertex.normal.SquareLength() >= FLT_MIN)) {
            continue;
        }
        vertex.normal = FbxVector4(0,0,0,1);
        if (onlyBroken) {
	...
	for (auto &triangle : triangles) {
        ...
        FbxVector4 faceNormal = this->getFaceNormal(triangle.verts);
        for (int vertIx : triangle.verts) {
